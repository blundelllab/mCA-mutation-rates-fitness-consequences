...
'''''
Watson code for creating heatmaps of confidence intervals for mCAs of a particular type that affect a particular gene.
Version 1.0 (August 2023)

Input:
    1) mCA type
    2) gene affected by mCA
...
Usage:
mCA_mu_and_s_MLE_using_heatmaps_length_dependence_genes.py --mCA_type --gene --lower_length --upper_length
...
def dataframe_df(gene, mCA_type):
...
def main():
    ...
    parser.add_argument("--gene", type=str, dest="gene", help="e.g. DNMT3A", required=True)
    ...
    labelname = mCA_type+' involving '+gene+' of length '+str(lower_length)+' to '+str(upper_length)+' MB'
    ...
    grid_size = df_dict[gene+'_'+str(lower_length)+'_'+str(upper_length)]['GRID']
    lower_s = float(df_dict[gene+'_'+str(lower_length)+'_'+str(upper_length)]['lower s'])
    upper_s = float(df_dict[gene+'_'+str(lower_length)+'_'+str(upper_length)]['upper s'])
    lower_mu = float(df_dict[gene+'_'+str(lower_length)+'_'+str(upper_length)]['lower mu'])
    upper_mu = float(df_dict[gene+'_'+str(lower_length)+'_'+str(upper_length)]['upper mu'])

    s_range = [lower_s, upper_s]
    mu_range = [lower_mu, upper_mu]

    #Step 2 = create list of logit cell fractions and cumulative densities
    data = dataframe_df(gene, mCA_type)
    log_cell_fractions, logit_cell_fractions = log_logit_CFs(gene, data, float(lower_length), float(upper_length))
    ...
    print('calculating s and mu (using heatmap) for '+mCA_type+' mCAs involing '+gene+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
    grid_size = 50
    ...
    print('calculating and plotting 95% confidence intervals for '+mCA_type+' mCAs involing '+gene+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
    ...
    mCA_results.write(dt_string+'\t'+gene+'_'+str(lower_length)+'-'+str(upper_length)+'MB'+'\t'+str(N_mCA)+'\t'+
    ...
    print('s = ', s)
    print('mu = ', mu)
    print('heatmap and confidence interval plots complete for '+mCA_type+' mCAs involing '+gene+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
