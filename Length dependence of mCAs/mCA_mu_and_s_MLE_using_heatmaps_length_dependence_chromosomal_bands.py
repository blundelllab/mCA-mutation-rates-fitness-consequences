...
'''''
Watson code for creating heatmaps of confidence intervals for mCAs of a particular type that affect a particular chromosomal band.
Version 1.0 (August 2023)

Input:
    1) mCA type
    2) chromosomal band affected by mCA
    3) lower range of mCA length (e.g. if want to just look at mCAs of length 0-3 MB, lower range = 0)
    4) upper range of mCA length (e.g. if want to justugust look at mCAs of length 0-3 MB, upper range = 3)

Usage:
mCA_mu_and_s_MLE_using_heatmaps_length_dependence_chromosomal_bands.py --mCA_type --band --lower_length --upper_length
...
def dataframe_df(band, mCA_type):
    ...
    df_mCA_type = filter_by_mCA_type(mCA_type, df_biobank) #filter to just include losses
...
def log_logit_CFs(band, df, lower_length, upper_length): #log and logit cell fractions
    ...
    print('total '+band+' mCAs length interval '+str(lower_length)+'-'+str(upper_length)+' = '+str(len(logitCFs)))
...
def heatmap_logit_cumulative(cumulative_densities_list, s_range, mu_range, grid_size, labelname, integral_limit):
    ...
    plt.tight_layout()
    plt.savefig('Figures/MLE_heatmaps_length_dependence_mCAs/'+savename+'_s_mu_heatmap.pdf')
    # plt.show()
...
def confidence_intervals_plot(s_xyz, mu_xyz, mCA, labelname):
    ...
    plt.tight_layout()
    plt.savefig('Figures/Confidence_intervals_length_dependence_mCAs/'+savename+'_s_mu_confidence_intervals_cumulative.pdf')
    # plt.show()
...
def plots_with_cumulative_MLE_logit(mCA, integral_limit, labelname, s, mu, logit_cell_fractions, total_people):
    ...
    savename = labelname.replace(' ', '_')
    plt.savefig('Figures/Cell_fraction_density_histograms_length_dependence_mCAs/'+str(savename)+'_cell_fraction_density_histogram_cumulative_logit.pdf')
...
def plots_with_cumulative_MLE(mCA, integral_limit, labelname, s, mu, log_cell_fractions, total_people):
    ...
    plt.savefig('Figures/Cell_fraction_density_histograms_length_dependence_mCAs/'+str(savename)+'_cell_fraction_density_histogram_cumulative.pdf')
...
def main():
    ...
    parser.add_argument("--mCA_type", type=str, dest="mCA_type", help="gain, loss or CNLOH", required=True)
    parser.add_argument("--band", type=str, dest="band", help="e.g. 14q32.33", required=True)
    parser.add_argument("--lower_length", type=str, dest="lower_length", help="e.g. 0 MB, put 0", required=True)
    parser.add_argument("--upper_length", type=str, dest="upper_length", help="e.g. 3 MB, put 3", required=True)
    o = parser.parse_args()

    band = o.band
    lower_length = o.lower_length
    upper_length = o.upper_length
    mCA_type = o.mCA_type
    ...
    labelname = mCA_type+' involving '+str(band)+' of length '+str(lower_length)+' to '+str(upper_length)+' MB'

    #Step 1 = read in grid ranges
    df_grid = pd.read_csv('Data_files/Heatmap_grid_ranges_length_dependence_mCAs.csv')
    df_grid = df_grid.set_index('mCA')
    df_dict = pd.DataFrame.to_dict(df_grid, orient = 'index')

    grid_size = df_dict[band+'_'+str(lower_length)+'_'+str(upper_length)]['GRID']
    lower_s = float(df_dict[band+'_'+str(lower_length)+'_'+str(upper_length)]['lower s'])
    upper_s = float(df_dict[band+'_'+str(lower_length)+'_'+str(upper_length)]['upper s'])
    lower_mu = float(df_dict[band+'_'+str(lower_length)+'_'+str(upper_length)]['lower mu'])
    upper_mu = float(df_dict[band+'_'+str(lower_length)+'_'+str(upper_length)]['upper mu'])

    s_range = [lower_s, upper_s]
    mu_range = [lower_mu, upper_mu]

    #Step 2 = create list of logit cell fractions and cumulative densities
    data = dataframe_df(band, mCA_type)
    log_cell_fractions, logit_cell_fractions = log_logit_CFs(band, data, float(lower_length), float(upper_length))
    ...
    print('calculating s and mu (using heatmap) for '+mCA_type+' mCAs involing '+band+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
    grid_size = 50
    s_xyz, mu_xyz, s, mu = heatmap_logit_cumulative(cumulative_densities_list, s_range, mu_range, grid_size, labelname, integral_limit)

    #Step 4 = calculate confidence intervals for s and mu
    print('calculating and plotting 95% confidence intervals for '+mCA_type+' mCAs involing '+band+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
    ...
    N_mCA = len(log_cell_fractions)

    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")

    mCA_results = open('Data_files/mCA_s_mu_with_confidence_intervals_length_dependence_mCAs.txt', 'a') #'a' means append to the file, rather than overwrite it
    mCA_results.write(dt_string+'\t'+band+'_'+str(lower_length)+'-'+str(upper_length)+'MB'+'\t'+str(N_mCA)+'\t'+
    ...
    print('s = ', s)
    print('mu = ', mu)
    print('heatmap and confidence interval plots complete for '+mCA_type+' mCAs involing '+band+' of length '+str(lower_length)+' - '+str(upper_length)+' MB')
