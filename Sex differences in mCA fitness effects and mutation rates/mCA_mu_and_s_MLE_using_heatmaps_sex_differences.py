...
def g(lower_interpolated_distribution, x):
    g_result = lower_interpolated_distribution(x) #female interpolated function
#     print('g = ', g_result)
    return g_result

def h(higher_interpolated_distribution, x, R):
    return higher_interpolated_distribution(x*R) #male interpolated function in terms of y = Rx (R = x/y)

def distribution_of_fold_difference(higher_interpolated_distribution, lower_interpolated_distribution, xnew, R):
    return integrate.quad(lambda x: g(lower_interpolated_distribution, x)*h(higher_interpolated_distribution, x, R), xnew[0], xnew[-1], limit=1000)[0] #integrate between lowest x-axis value and highest x-axis value

def p_value(higher_interpolated_distribution, lower_interpolated_distribution, xnew): #where lower_D is the lower limit to integrate down to (i.e. p-value = area under the curve between lower D and 0)
    ...
    full_area = integrate.quad(lambda R: distribution_of_fold_difference(higher_interpolated_distribution, lower_interpolated_distribution, xnew, R), 0, upper_full_area, limit=10000)[0] #integrate between 0 and upper fold difference
    under_one_area = integrate.quad(lambda R: distribution_of_fold_difference(higher_interpolated_distribution, lower_interpolated_distribution, xnew,  R), 0, 1, limit=10000)[0] #integrate between 0 and 1
    print('full area = ', full_area)
    print('under one area = ', under_one_area)
    return under_one_area/full_area
...
def plot_distribution_differences_with_95_CI(xnew, higher_interpolated_distribution, lower_interpolated_distribution, p_val, s_or_mu, mCA, ax):
    ...
    xx=np.array([R for R in np.linspace(0, upper_full_area, 10000)])
    yy=[distribution_of_fold_difference(higher_interpolated_distribution, lower_interpolated_distribution, xnew, R) for R in xx]

    ax.fill_between(xx, yy, where= xx<=1, zorder = 0, facecolor = 'white', hatch = '///')
    ax.plot(xx, yy, color = grey4, lw = 2, zorder = 1)
    ax.plot([1, 1], [0, max(yy)], color = grey4, lw = 2, zorder = 70)
    ...
    ax.set_xlabel(s_or_mu+' fold difference for '+mCA, fontsize = 14)
...
def calculate_p_value(s_or_mu, mCA, male_s_or_mu, male_s_or_mu_list, male_probs_list, female_s_or_mu, female_s_or_mu_list, female_probs_list, labelname):
    ...
    female_interpolated = interp1d(female_s_or_mu_list, female_probs_list, fill_value=0, bounds_error=False) #interpolate the female distribution
    male_interpolated = interp1d(male_s_or_mu_list, male_probs_list, fill_value=0, bounds_error=False) #interpolate the female distribution

    #Step 2 = Determine which distribution is the one with the suspected higher s or mu value:
    if female_s_or_mu >= male_s_or_mu:
        higher_interpolated_distribution = female_interpolated
        lower_interpolated_distribution = male_interpolated
    else:
        higher_interpolated_distribution = male_interpolated
        lower_interpolated_distribution = female_interpolated
    ...
    xnew = np.linspace(min_x, max_x, num=100000, endpoint=True) #full range of x-axis values for the interpolated male and female distributions
    p_val = p_value(higher_interpolated_distribution, lower_interpolated_distribution, xnew)
    print('p-value = ', p_val)
    ...
    ax1 = axes[0]
    ax2 = axes[1]
    plot_distributions_with_interpolation(female_s_or_mu_list, female_probs_list, female_interpolated, male_s_or_mu_list, male_probs_list, male_interpolated, s_or_mu, mCA, ax1)
    ...
    # plt.show()
    plt.savefig('Figures/P_value_plots_sex_fold_differences_mCAs/'+str(savename)+'_distribution_sex_fold_differences_p_value_calculation_'+s_or_mu+'.pdf')
...
def main():
    ...
    #STEP 1 = calculate the p-value for fold difference for fitness effects
    ...
    #STEP 2 = calculate the p-value for fold difference for mutation rates
    ...
    print('p-value for s for '+mCA+'= '+str(p_val_s))
    print('lower 95% CI for s fold-difference for '+mCA+'= '+str(lower_CI_difference_s))
    print('upper 95% CI for s fold-difference for '+mCA+'= '+str(upper_CI_difference_s))
    print('male mu = ', male_mu)
    print('female mu = ', female_mu)
    print('p-value for mu for '+mCA+'= '+str(p_val_mu))
    print('lower 95% CI for mu fold-difference for '+mCA+'= '+str(lower_CI_difference_mu))
    print('upper 95% CI for mu fold-difference for '+mCA+'= '+str(upper_CI_difference_mu))
    ...
    print('heatmap and confidence interval plots complete for sex fold differences in '+str(mCA)+' in '+str(int(time.time() - start_time)/3600)+' hours')
