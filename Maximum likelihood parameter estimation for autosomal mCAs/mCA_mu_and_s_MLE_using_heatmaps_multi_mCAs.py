...
'''''
Watson code for estimating mCA mutation rate and fitness effects (and their confidence intervals) using MLE heatmaps (for mCAs with additional mCAs).
...
Usage:
mCA_mu_and_s_MLE_using_heatmaps_multi_mCAs.py  --mCA name of mCA
...
def filter_by_more_than_1_mCAs(dataframe): #e.g. filter by those that only have 1 total mCA
    mask = dataframe['total_mCAs']>=2
...
def log_logit_CFs(df, mCA): #log and logit cell fractions
    multiple_df = filter_by_more_than_1_mCAs(df) #filter the dataframe to only include people that have total 1 mCA
    CFs_mCA = multiple_df[multiple_df['annotation']== mCA]['CELL_FRAC'].to_list()
    ...
    print('total '+mCA+' mCAs = '+str(len(logitCFs)))
...
def heatmap_logit_cumulative(cumulative_densities_list, s_range, mu_range, grid_size, mCA, labelname, integral_limit):
    ...
    plt.tight_layout()
    plt.savefig('Figures/MLE_heatmaps_multiple_mCAs/'+savename+'_multiple_s_mu_heatmap.pdf')
    # plt.show()
...
def confidence_intervals_plot(s_xyz, mu_xyz, mCA, labelname):
    ...
    plt.tight_layout()
    plt.savefig('Figures/Confidence_intervals_multiple_mCAs/'+savename+'_multiple_s_mu_confidence_intervals_cumulative.pdf')
    # plt.show()
...
def plots_with_cumulative_MLE_logit(mCA, integral_limit, labelname, s, mu, logit_cell_fractions, total_people):
    ...
    savename = labelname.replace(' ', '_')
    plt.savefig('Figures/Cell_fraction_density_histograms_individual_mCAs/'+str(savename)+'_cell_fraction_density_histogram_cumulative_logit.pdf')
...
def plots_with_cumulative_MLE(mCA, integral_limit, labelname, s, mu, log_cell_fractions, total_people):
    ...
    plt.savefig('Figures/Cell_fraction_density_histograms_multiple_mCAs/'+str(savename)+'_multiple_cell_fraction_density_histogram_cumulative.pdf')
...
def main():
    ...
    if mCA[-1]=='+':
        labelname = 'gain '+str(mCA[:-1])+' (+ additional mCA(s))'
        integral_limit = 0.999
    if mCA[-1]=='-':
        labelname = 'loss '+str(mCA[:-1])+' (+ additional mCA(s))'
        integral_limit = 0.67
    if mCA[-1]=='=':
        labelname = 'CNLOH '+str(mCA[:-1])+' (+ additional mCA(s))'
        integral_limit = 0.54

    #Step 1 = read in grid ranges
    df_grid = pd.read_csv('Data_files/Heatmap_grid_ranges_mCAs_multiple.csv')
    ...
    log_cell_fractions, logit_cell_fractions = log_logit_CFs(df, mCA)
    ...
    N_single_mCA = mCA_count_number_mCAs(mCA, df, 1)
    N_double_mCAs = mCA_count_number_mCAs(mCA, df, 2)
    N_three_or_more_mCAs = mCA_count_greater_than_equal_number_mCAs(mCA, df, 3)

    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")

    mCA_results = open('Data_files/mCA_s_mu_with_confidence_intervals_individual_mCAs_multiple_mCAs.txt', 'a') #'a' means append to the file, rather than overwrite it
    mCA_results.write(dt_string+'\t'+mCA+'\t'+str(N_single_mCA)+'\t'+str(N_double_mCAs)+'\t'+str(N_three_or_more_mCAs)+'\t'+
    ...
    print('s = ', s)
    print('mu = ', mu)
    print('heatmap and confidence interval plots complete for '+str(mCA)+' (multiple mCAs) in '+str(int(time.time() - start_time)/3600)+' hours')
