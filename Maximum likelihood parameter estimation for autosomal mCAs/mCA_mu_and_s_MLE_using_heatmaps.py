...
'''''
Watson code for estimating individual mCA mutation rate and fitness effects (and their confidence intervals) using MLE heatmaps.
...
Usage:
mCA_mu_and_s_MLE_usinusing_heatmaps.py  --mCA name of mCA
...
def heatmap_logit_cumulative(cumulative_densities_list, s_range, mu_range, grid_size, mCA, labelname, integral_limit):
    ...
    plt.tight_layout()
    plt.savefig('Figures/MLE_heatmaps_individual_mCAs/'+savename+'_s_mu_heatmap.pdf')
    # plt.show()
...
def confidence_intervals_plot(s_xyz, mu_xyz, mCA, labelname):
    ...
    plt.tight_layout()
    plt.savefig('Figures/Confidence_intervals_individual_mCAs/'+savename+'_s_mu_confidence_intervals_cumulative.pdf')
    # plt.show()
...
def main():
    ...
    df_grid = pd.read_csv('Data_files/Heatmap_grid_ranges_individual_mCAs.csv')
    ...
    mCA_results = open('Data_files/mCA_s_mu_with_confidence_intervals_individual_mCAs.txt', 'a') #'a' means append to the file, rather than overwrite it
