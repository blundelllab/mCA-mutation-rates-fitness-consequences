...
'''''
Watson code for creating heatmaps of confidence intervals for mCAs, with cell fractions trimmed below a lower cell fraction limit of detection.
...
Usage:
mCA_mu_s_MLE_using_heatmaps_with_lower_trimming.py  --mCA name of mCA
...
def mCA_count_number_mCAs_with_trimming(df, mCA, trimming_cell_fraction):
    single_df = filter_by_number_mCAs(1, df) #filter the dataframe to only include people that have total 1 mCA
    CFs_mCA = single_df[single_df['annotation']== mCA]['CELL_FRAC'].to_list()

    CF_count = 0
    for i in CFs_mCA:
        if float(i) >=float(trimming_cell_fraction):
            CF_count+=1
    return CF_count

def log_logit_CFs_with_trimming(df, mCA, trimming_cell_fraction): #log and logit cell fractions
    single_df = filter_by_number_mCAs(1, df) #filter the dataframe to only include people that have total 1 mCA
    CFs_mCA = single_df[single_df['annotation']== mCA]['CELL_FRAC'].to_list()
...
def heatmap_logit_cumulative(cumulative_densities_list, s_range, mu_range, grid_size, mCA, labelname, integral_limit):
    ...
    plt.tight_layout()
    plt.savefig('Figures/MLE_heatmaps_individual_mCAs_with_lower_trimming/'+savename+'_s_mu_heatmap.pdf')
    # plt.show()
...
def confidence_intervals_plot(s_xyz, mu_xyz, mCA, labelname):
    ...
    plt.tight_layout()
    plt.savefig('Figures/Confidence_intervals_individual_mCAs_with_lower_trimming/'+savename+'_s_mu_confidence_intervals_cumulative.pdf')
    # plt.show()
...
def plots_with_cumulative_MLE_logit(mCA, integral_limit, labelname, s, mu, logit_cell_fractions, total_people):
    ...
    savename = labelname.replace(' ', '_')
    plt.savefig('Figures/Cell_fraction_density_histograms_individual_mCAs_with_lower_trimming/'+str(savename)+'_cell_fraction_density_histogram_cumulative_logit.pdf')
...
def plots_with_cumulative_MLE(mCA, integral_limit, labelname, s, mu, log_cell_fractions, total_people):
    ...
    plt.savefig('Figures/Cell_fraction_density_histograms_individual_mCAs_with_lower_trimming/'+str(savename)+'_cell_fraction_density_histogram_cumulative.pdf')
...
def main():
    ...
    if mCA[-1]=='+':
        ...
        trimming_cell_fraction = 0.02576549772271803
    if mCA[-1]=='-':
        labelname = 'loss '+str(mCA[:-1])
        integral_limit = 0.67
        trimming_cell_fraction = 0.04825906550193745
    if mCA[-1]=='=':
        labelname = 'CNLOH '+str(mCA[:-1])
        integral_limit = 0.54
        trimming_cell_fraction = 0.015448664542784136

    #Step 1 = read in grid ranges
    df_grid = pd.read_csv('Data_files/Heatmap_grid_ranges_individual_mCAs_with_lower_trimming.csv')
    ...
    log_cell_fractions, logit_cell_fractions = log_logit_CFs_with_trimming(df, mCA, trimming_cell_fraction)
    ...
    N_single_mCA = mCA_count_number_mCAs_with_trimming(df, mCA, trimming_cell_fraction)

    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")

    mCA_results = open('Data_files/mCA_s_mu_with_confidence_intervals_individual_mCAs_with_lower_cell_fraction_trimming.txt', 'a') #'a' means append to the file, rather than overwrite it
    mCA_results.write(dt_string+'\t'+mCA+'\t'+str(N_single_mCA)+'\t'+
