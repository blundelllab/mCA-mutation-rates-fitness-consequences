...
def log_logit_cell_fractions_CF(df, lower_limit, upper_limit):
    ...
    for i in CFs_mCA:
        if lower_limit <= (i) <= upper_limit:
            logitCFs.append(scipy.special.logit(i)) #logit
            logCFs.append(np.log(i))
...
#         shortened_logit_list.append(v[-1])
#         list_length = len(v)
# #         if list_length>=2:
# #             shortened_logit_list.append(v[int(len(v)/2)])
#
#     if list_length>1:
#         shortened_logit_list.append(sorted_logit_cell_fractions[0]) #make sure the final datapoint is in the list (i.e. both first and last will be there)
#
#     return shortened_logit_list
...
def ProbDataGivenModel_logit_cumulative(params, logit_cell_fractions, data, integral_limit):

    predicted_cumulative = [Probtheory_logit_cumulative(integral_limit, l, params) for l in logit_cell_fractions]

    x_y_dict = {}
    for a, b in zip(logit_cell_fractions, predicted_cumulative):
        if math.isnan(b) == False:
            x_y_dict[a]=b #a dictionary of the x and y datapoints of the cumulative theory (but only of the sampled logit spaced cell fractions)
...
def heatmap_logit_cumulative_subsample(cumulative_densities_list, s_range, mu_range, grid_size, mCA, labelname, integral_limit):
    ...
    if len(x)>50:
        ...
        logProbs = np.array([[ProbDataGivenModel_logit_cumulative([s, mu], x, cumulative_densities_list, integral_limit) for s in s_list] for mu in mu_list])
    ...
    if len(x)>50:
        ...
        logProbs = np.array([[ProbDataGivenModel_logit_cumulative([s, mu], x, cumulative_densities_list, integral_limit) for mu in mu_list] for s in s_list])
    ...
    plt.tight_layout()
    plt.savefig('Figures/Heatmaps_simulated_mCAs/'+savename+'_s_mu_heatmap.pdf')
    # plt.show()
...
def confidence_intervals_plot(s_xyz, mu_xyz, mCA, labelname):
    ...
    plt.tight_layout()
    plt.savefig('Figures/Confidence_intervals_simulated_mCAs/'+savename+'_s_mu_confidence_intervals_cumulative.pdf')
    # plt.show()
...
def plots_with_cumulative_MLE(mCA_type, integral_limit, labelname, s, mu, log_cell_fractions, total_people):
    ...
    plot_data_non_cumulative(ax1, mu, log_cell_fractions, total_people, color, mCA_type)
    plot_data_cumulative(ax2, mu, log_cell_fractions, total_people, color, mCA_type)
    ...
    plt.savefig('Figures/Cell_fraction_density_histograms_individual_mCAs/'+str(savename)+'_cell_fraction_density_histogram_cumulative.pdf')
...
def plots_with_cumulative_MLE_logit_subsample(mCA_type, integral_limit, labelname, s, mu, logit_cell_fractions, total_people):
    ...
    plt.tight_layout(w_pad = 5)
    savename = labelname.replace(' ', '_')
    plt.savefig('Figures/Cell_fraction_density_histograms_individual_mCAs_logit/'+str(savename)+'_cell_fraction_density_histogram_logit_cumulative.pdf')
...
def main():
    ...
    df_for_ranges = pd.read_csv('Data_files/Heatmap_grid_ranges_simulated_mCAs.csv', sep = ',')
    ...
    df = pd.read_csv('Simulation_results_v2/Simulated_mCAs/biobank_sim_test_u=4.35e-09_s_'+str(fitness)+'01_N100000_u_mCA=4.35e-09_seed3_biobank_variants.csv')

    log_cell_fractions, logit_cell_fractions = log_logit_cell_fractions_CF(df, lower_limit, integral_limit)
    N = len(logit_cell_fractions)
    print('simulated mCA = '+str(fitness)+' '+mCA_type+' '+capped_or_not)
    ...
    if len(logit_cell_fractions)>50:
        plots_with_cumulative_MLE_logit_subsample(mCA_type, integral_limit, labelname, s, mu, logit_cell_fractions, total_people)
    plots_with_cumulative_MLE(mCA_type, integral_limit, labelname, s, mu, log_cell_fractions, total_people)

    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")

    fitness_results = open('Data_files/Simulation_s_mu_inferences.txt', 'a') #'a' means append to the file, rather than overwrite it
